...
test_MCTS = True
all_results = False
rollout_results = False

"""Make plots of Agent vs Agent"""
for board_size in board_sizes:
    if test_MCTS or all_results:
    ...
    if test_MCTS or all_results:
        ...
        time_list.append(end - start)
        game_plots(MCTS_MCTS_games, test_iters, "MCTS 1", "MCTS 2",
                   f"MCTS_MCTS_{test_iters}_{num_roll_outs}_{board_size}")
    ...
    if all_results:
        ...
        start = time()
        for i in range(test_iters):
            winner = MCTS_v_Minimax(10, player_id=player_ids[i], return_winner=True, data_run=True,
                                    board_size=board_size)
            if (winner == 2) or (player_ids[i] == 0):
    ...
    if test_MCTS or all_results:
        ...
        start = time()
        for i in range(test_iters):
            winner = Expectimax_v_Random(player_id=player_ids[i], return_winner=True, data_run=True,
                                         board_size=board_size)
            if (winner == 2) or (player_ids[i] == 0):
    ...
    if test_MCTS or all_results:
        ...
        start = time()
        for i in range(test_iters):
            winner = MCTS_v_Epectimax(10, player_id=player_ids[i], return_winner=True, data_run=True,
                                      board_size=board_size)
            if (winner == 2) or (player_ids[i] == 0):
...
if rollout_results:
    for board_size in board_sizes:
        ...
        ctr = 0
        for iters in range(1, max_roll_outs + 1):
            for i in range(test_iters):
                if ctr == 0:
